...
public class svlParticipacion extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ( id != null ) {
			...
			/*
			String[][] mDPD = {{"DEPARTAMENTO","CONTINENTE"},{"PROVINCIA", "PAIS"},{"DISTRITO","CIUDAD"}};
			sDPD = (bNacional ? mDPD[ tID - 1][0] : mDPD[ tID - 1][1]).toUpperCase(); 
			*/
			if ( tID == 1) sDPD = bNacional ? "DEPARTAMENTO " : " CONTINENTE"; //tid = tamaño del id
			else if ( tID  == 2) sDPD = bNacional ? "PROVINCIA " : " PAÏS";
